...
def create(dataset_frame_path,trackNet_test_folder_videos):
    ...
    for video_name in os.listdir(trackNet_test_folder_videos):
        ...
        last_frame_ball_position = None if ball_track[-1] == (None,None) else (int(float(ball_track[-1][0])*width/640),int(float(ball_track[-1][1])*height/360))
        
        frame_name = get_frame_name(video_name)
        
        if last_frame_ball_position is not None:
            print(last_frame_ball_position)
